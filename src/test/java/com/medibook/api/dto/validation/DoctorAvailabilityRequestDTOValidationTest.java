...
class DoctorAvailabilityRequestDTOValidationTest {
    ...
    void emptyWeeklyAvailability_ShouldPass() {
        // Arrange - Empty list is valid, just no availability
    ...
    @Test
    void invalidDayInWeeklyAvailability_ShouldFail() {
        // Arrange
        List<DayAvailabilityDTO> weeklyAvailability = List.of(
            new DayAvailabilityDTO("MONDAY", true, List.of(
                new TimeRangeDTO("09:00", "17:00")
            )),
            new DayAvailabilityDTO("", false, Collections.emptyList()), // Invalid day
            new DayAvailabilityDTO("WEDNESDAY", null, Collections.emptyList()) // Invalid enabled
        );
    ...
    @Test
    void invalidTimeRangeInWeeklyAvailability_ShouldFail() {
        // Arrange
        List<DayAvailabilityDTO> weeklyAvailability = List.of(
            new DayAvailabilityDTO("MONDAY", true, List.of(
                new TimeRangeDTO("25:00", "17:00"), // Invalid start time
                new TimeRangeDTO("09:00", "24:60")  // Invalid end time
            ))
        );
    ...
    void multipleViolations_ShouldReturnAllViolations() {
        ...
        List<DayAvailabilityDTO> weeklyAvailability = List.of(
            new DayAvailabilityDTO("", null, List.of( // Invalid day and enabled
                new TimeRangeDTO("", "25:00") // Invalid start and end time
            ))
        );
        
        DoctorAvailabilityRequestDTO request = new DoctorAvailabilityRequestDTO(-5, weeklyAvailability); // Invalid slot duration
        ...
        // Assert
        assertFalse(violations.isEmpty());
        assertTrue(violations.size() >= 4); // At least 4 violations expected
        
        // Check for specific violation messages
        assertTrue(violations.stream().anyMatch(v -> v.getMessage().contains("Slot duration must be at least 5 minutes")));
        assertTrue(violations.stream().anyMatch(v -> v.getMessage().contains("Day is required")));
        assertTrue(violations.stream().anyMatch(v -> v.getMessage().contains("Enabled status is required")));
    ...
    void complexValidAvailability_ShouldPass() {
        // Arrange - Complex but valid availability with different patterns
        List<DayAvailabilityDTO> weeklyAvailability = List.of(
            new DayAvailabilityDTO("MONDAY", true, List.of(
            ...
            new DayAvailabilityDTO("TUESDAY", true, List.of(
                new TimeRangeDTO("00:00", "23:59") // Full day
            )),
            new DayAvailabilityDTO("WEDNESDAY", false, Collections.emptyList()),
            new DayAvailabilityDTO("THURSDAY", true, List.of(
                new TimeRangeDTO("7:00", "8:00") // Single digit hour
            )),
            new DayAvailabilityDTO("FRIDAY", true, List.of(
                new TimeRangeDTO("09:00", "09:30") // 30-minute slot
            )),
            new DayAvailabilityDTO("SATURDAY", false, Collections.emptyList()),
            new DayAvailabilityDTO("SUNDAY", false, Collections.emptyList())
        );
