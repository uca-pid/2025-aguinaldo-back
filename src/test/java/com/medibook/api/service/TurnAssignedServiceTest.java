...
class TurnAssignedServiceTest {
    ...
    @Test
    void getTurnsNeedingRating_ReturnsOnlyPastUnratedTurns() {
        // Arrange
        UUID patientId = UUID.randomUUID();
        
        // Past turn without rating (should be included)
        TurnAssigned pastTurnUnrated = TurnAssigned.builder()
                .id(UUID.randomUUID())
                .patient(patient)
                .doctor(doctor)
        ...
        // Past turn with rating (should be excluded)
        TurnAssigned pastTurnRated = TurnAssigned.builder()
                .id(UUID.randomUUID())
                .patient(patient)
                .doctor(doctor)
                .scheduledAt(OffsetDateTime.now().minusDays(3))
                .status("COMPLETED")
                .build();
        
        // Future turn (should be excluded)
        ...
        // Canceled turn (should be excluded)
        TurnAssigned canceledTurn = TurnAssigned.builder()
                .id(UUID.randomUUID())
                .patient(patient)
                .doctor(doctor)
        ...
        List<TurnAssigned> allTurns = List.of(pastTurnUnrated, pastTurnRated, futureTurn, canceledTurn);
        
        when(turnRepo.findByPatient_IdOrderByScheduledAtDesc(patientId)).thenReturn(allTurns);
        when(ratingRepo.existsByTurnAssigned_IdAndRater_Id(pastTurnUnrated.getId(), patientId)).thenReturn(false);
        when(ratingRepo.existsByTurnAssigned_IdAndRater_Id(pastTurnRated.getId(), patientId)).thenReturn(true);
        when(ratingRepo.existsByTurnAssigned_IdAndRater_Id(futureTurn.getId(), patientId)).thenReturn(false);
        when(ratingRepo.existsByTurnAssigned_IdAndRater_Id(canceledTurn.getId(), patientId)).thenReturn(false);
        
        TurnResponseDTO expectedResponse = TurnResponseDTO.builder()
                .id(pastTurnUnrated.getId())
                .doctorId(doctor.getId())
                .doctorName("Dr. Juan Pérez")
                .doctorSpecialty("Cardiology")
                .patientId(patient.getId())
                .patientName("Ana García")
                .scheduledAt(pastTurnUnrated.getScheduledAt())
                .status("COMPLETED")
                .build();
        when(mapper.toDTO(pastTurnUnrated)).thenReturn(expectedResponse);

        // Act
        List<TurnResponseDTO> result = turnAssignedService.getTurnsNeedingRating(patientId);

        // Assert
        assertEquals(1, result.size());
        assertEquals(expectedResponse, result.get(0));
        verify(mapper).toDTO(pastTurnUnrated);
        verify(mapper, never()).toDTO(pastTurnRated);
        verify(mapper, never()).toDTO(futureTurn);
        verify(mapper, never()).toDTO(canceledTurn);
    }

    @Test
    void getTurnsNeedingRating_NoTurns_ReturnsEmptyList() {
        // Arrange
        UUID patientId = UUID.randomUUID();
        when(turnRepo.findByPatient_IdOrderByScheduledAtDesc(patientId)).thenReturn(List.of());
    ...
    @Test
    void getTurnsNeedingRating_AllTurnsAlreadyRated_ReturnsEmptyList() {
        // Arrange
        UUID patientId = UUID.randomUUID();
        
        TurnAssigned ratedTurn1 = TurnAssigned.builder()
                .id(UUID.randomUUID())
                .patient(patient)
                .doctor(doctor)
                .scheduledAt(OffsetDateTime.now().minusDays(1))
        ...
        TurnAssigned ratedTurn2 = TurnAssigned.builder()
                .id(UUID.randomUUID())
                .patient(patient)
                .doctor(doctor)
                .scheduledAt(OffsetDateTime.now().minusDays(2))
        ...
        List<TurnAssigned> allTurns = List.of(ratedTurn1, ratedTurn2);
        
        when(turnRepo.findByPatient_IdOrderByScheduledAtDesc(patientId)).thenReturn(allTurns);
        when(ratingRepo.existsByTurnAssigned_IdAndRater_Id(ratedTurn1.getId(), patientId)).thenReturn(true);
        when(ratingRepo.existsByTurnAssigned_IdAndRater_Id(ratedTurn2.getId(), patientId)).thenReturn(true);
    ...
    @Test
    void getTurnsNeedingRating_CancelledSpellingVariants_ExcludesBoth() {
        // Arrange
        UUID patientId = UUID.randomUUID();
        
        // Turn with "CANCELED" status
        TurnAssigned canceledTurn = TurnAssigned.builder()
                .id(UUID.randomUUID())
                .patient(patient)
                .doctor(doctor)
        ...
        // Turn with "CANCELLED" status  
        TurnAssigned cancelledTurn = TurnAssigned.builder()
                .id(UUID.randomUUID())
                .patient(patient)
                .doctor(doctor)
                .scheduledAt(OffsetDateTime.now().minusDays(2))
        ...
        List<TurnAssigned> allTurns = List.of(canceledTurn, cancelledTurn);
